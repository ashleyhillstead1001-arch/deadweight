 ...
 * Handle user logout.
 *
 * NOTE: deadweight.sid is our configured session cookie name (set via the
 * `name` option in server.js), so we clear that cookie on logout.
 */
const processLogout = (req, res) => {
    ...
    req.session.destroy((err) => {
        if (err) {
             ...
             */
            res.clearCookie('deadweight.sid');
